...
import '../supabase/sync_orchestrator.dart';
...
class ChatService extends ChangeNotifier {
  ...
  Future<void> deleteConversation(String id) async {
    ...
    // ===== Supabase sync hook =====
    try {
      SyncOrchestrator.instance.enqueueThreadDeletion(id);
    } catch (_) {}
  }
  ...
  Future<void> renameConversation(String id, String newTitle) async {
    ...
    // ===== Supabase sync hook =====
    try {
      SyncOrchestrator.instance.enqueueThread(id);
    } catch (_) {}
  }
  ...
  Future<ChatMessage> addMessage({
    ...
    // ===== Supabase sync hook (only when streaming is done) =====
    if (!isStreaming) {
      try {
        SyncOrchestrator.instance.enqueueThread(conversationId);
      } catch (_) {}
    }
  ...
  Future<void> updateMessage(
    ...
    // ===== Supabase sync hook (when streaming finishes) =====
    if (isStreaming == false) {
      try {
        SyncOrchestrator.instance.enqueueThread(conversationId);
      } catch (_) {}
