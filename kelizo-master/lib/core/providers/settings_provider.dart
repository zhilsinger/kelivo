...
class SettingsProvider extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    if (_providerConfigs.isEmpty) {
      ...
      ensureProviderConfig('KelizoIN', defaultName: 'KelizoIN');
  ...
  Future<void> setAppFontFromLocal({
    ...
    final fam = await _registerLocalFont(
      path: path,
      aliasPrefix: alias ?? 'kelizo_local_app',
    );
    if (fam == null) return;
    _appFontIsGoogle = false;
    _appFontFamily = fam;
  ...
  Future<void> setCodeFontFromLocal({
    ...
    final fam = await _registerLocalFont(
      path: path,
      aliasPrefix: alias ?? 'kelizo_local_code',
    );
    if (fam == null) return;
    _codeFontIsGoogle = false;
    _codeFontFamily = fam;
  ...
  Future<void> _reloadLocalFontsIfAny() async {
    ...
    if (_appFontLocalPath != null && _appFontLocalPath!.isNotEmpty) {
      final alias = _appFontLocalAlias ?? 'kelizo_local_app';
      final fam = await _registerLocalFont(
    ...
    if (_codeFontLocalPath != null && _codeFontLocalPath!.isNotEmpty) {
      final alias = _codeFontLocalAlias ?? 'kelizo_local_code';
      final fam = await _registerLocalFont(
...
class ProviderConfig {
  ...
  static String _defaultBase(String key) {
    ...
    if (k.contains('kelizoin')) return 'https://text.pollinations.ai/openai';
  ...
  static ProviderConfig defaultsFor(String key, {String? displayName}) {
    bool defaultEnabled(String k) {
      ...
      if (s.contains('kelizoin')) return true;
    ...
    switch (kind) {
      ...
      case ProviderKind.openai:
        // Special-case KelizoIN default models and overrides
        if (lowerKey.contains('kelizoin')) {
          return ProviderConfig(
            id: key,
            enabled: defaultEnabled(key),
            name: displayName ?? key,
